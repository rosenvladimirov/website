...
odoo.define('website_sale.website_sale', function (require) {
    ...
    $('.oe_website_sale').each(function () {
        ...
        $(oe_website_sale).on('click', 'a.js_add_package_cart_json', function (ev) {
            ev.preventDefault();
            var $link = $(ev.currentTarget);
            var $input = $link.parent().find("#add_qty_package");
            var $input_qty = $('input[name="add_qty"]');
            var product_id = +$input_qty.closest('*:has(input[name="product_id"])').find('input[name="product_id"]').val();
            var min = parseFloat($input.data("min") || 0);
            var max = parseFloat($input.data("max") || Infinity);
            var package_qty = parseFloat($input.children('option:selected').attr("data-qty"));
            var current_qty = isNaN($input_qty.val()) ? 0 : $input_qty.val();
            package_qty= isNaN(package_qty) ? 1 : package_qty;
            current_qty = (current_qty/package_qty)-(current_qty/package_qty>>0)>0 ? (current_qty/package_qty>>0)*package_qty : current_qty;
            var quantity = ($link.has(".fa-minus").length ? -package_qty : package_qty) + parseFloat(current_qty || 0, 10);
            var new_qty = quantity < 0 ? 0 : quantity;
            // if they are more of one input for this product (eg: option modal)
            $input_qty.add($input_qty).filter(function () {
        ...
        $("#add_qty_package").change(function() {
            var $input_qty = $('input[name="add_qty"]');
            var $pack_qty = $('#add_qty_package option:selected').attr("data-qty");
            $pack_qty = isNaN($pack_qty) ? 1 : $pack_qty;

            if (isNaN($input_qty.val())){
                $input_qty.val("1").change();
            } else {
                $input_qty.val(Math.round($pack_qty)).change();
            }
    ...
    $(document).ready(function() {
        var $input_qty = $('input[name="add_qty"]');
        var product_id = +$input_qty.closest('*:has(input[name="product_id"])').find('input[name="product_id"]').val();
        var $input = $("#add_qty_package");
        var package_qty = parseFloat($input.children('option:selected').attr("data-qty"));
        console.log("Packages", package_qty);
        if (!isNaN(package_qty)) {
            $input_qty.prev().css("display","none");
            $input_qty.next().css("display","none");
            $input_qty.add($input_qty).filter(function () {
                var $prod = $(this).closest('*:has(input[name="product_id"])');
                return !$prod.length || +$prod.find('input[name="product_id"]').val() === product_id;
            }).val(package_qty).change();
        }
    });
