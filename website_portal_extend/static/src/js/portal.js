...
'use strict';
    ...
    if ($('.o_portal_details').length) {
        ...
        $('.o_portal_details').on('change', "select[name='country_id']", function () {
            ...
            var nb = displayed_state.appendTo(select_state).show().size();
            select_state.parent().toggle(nb>=1);
            var select_city = $("select[name='city_id']");
            city_options.detach();
            var displayed_city = city_options.filter("[data-country_id="+($(this).val() || 0)+"]");
            var nb = displayed_city.appendTo(select_city).show().size();
            select_city.parent().toggle(nb>=1);
        });
    }
