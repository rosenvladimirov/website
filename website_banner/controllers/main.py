...
class WebsiteBanner(http.Controller):
    ...
    def index(self, **kw):
        ...
        if promos:
            for promo in promos:
                start_date = datetime.strptime(promo.start_date, '%Y-%m-%d %H:%M:%S').date()
                end_date = datetime.strptime(promo.end_date, '%Y-%m-%d %H:%M:%S').date()
                ...
                if current_date >= start_date and current_date <= end_date:
                    cont+="""
<h2 id="text" style="text-align:center">"""+promo.name+"""</h1>      
<p id="text2" style="text-align:center">"""+promo.text_to_display+"""</p>
<a href='"""+promo.url+"""' class="btn btn-primary" id="button" style="text-align:center" >Click to View</a>
<br /><br />
"""
